	...
	service("Profile", ['$http', 'notifications', 
		function($http, notifications){
			...
			var friendlistAdd = function(item) {
				...
				var url = '/api/user/:username/friendlist'.replace(':username', username);
		
				return $http.post(url, item).success(function(data) {
					if (data.notAdded) {
			...
			var bioUpdate = function(bioItem) {
				...
				var url = '/api/user/:username/bio'.replace(':username', username);
			...
			var bandUpdate = function(bandItem) {
				...
				var url = '/api/user/:username/bands'.replace(':username', username);
