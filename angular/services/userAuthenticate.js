angular.
	...
	service("userAuth", ['$http', '$window', '$location', 'notifications',
		function($http, $window, $location, notifications){
			...
			//Searches the database for a queried user
			var userSearch = function(user) {
				var username = encodeURIComponent(user);
		
				var url = '/api/user/:username'.replace(':username', username);
		
				return $http.get(url).then(function(res) {
					if (res.data.noUser) {
						throw new Error(res.data.noUser);
					} else {
			...
			//returns the info on the queried user
